...
export default function FarmerDApp() {
  ...
  const generateQRAndSubmit = async () => {
    ...
    try {
      ...
      const existingSharedBatches = JSON.parse(localStorage.getItem('herbledgerBatches') || '[]')
      existingSharedBatches.push(batchData)
      localStorage.setItem('herbledgerBatches', JSON.stringify(existingSharedBatches))
      ...
      try {
        ...
        const localData = JSON.parse(localStorage.getItem('herbledger-pending-submissions') || '[]')
        localData.push({
          ...collectionEventData,
          submittedAt: new Date().toISOString(),
          syncStatus: 'pending'
        })
        localStorage.setItem('herbledger-pending-submissions', JSON.stringify(localData))
      }
  ...
  if (!user) {
    if (showSignup) {
      return (
        <>
          <Head>
            <title>HERBLEDGER - {t('farmerInformation', currentLanguage)}</title>
    ...
    return (
      <>
        <Head>
          <title>HERBLEDGER - Farmer Portal Login</title>
  ...
  if (showProfile) {
    return (
      <>
        <Head>
          <title>HERBLEDGER - Farmer Profile</title>
  ...
  if (showBatchTracking && selectedBatchId) {
    return (
      <>
        <Head>
          <title>HERBLEDGER - Batch Tracking</title>
  ...
  if (user && showDashboard) {
    return (
      <>
        <Head>
          <title>HERBLEDGER - Farmer Dashboard</title>
            ...
            <div className="flex items-center justify-between max-w-7xl mx-auto">
              <div className="flex items-center space-x-2 sm:space-x-4">
                ...
                <div className="min-w-0">
                  <h1 className="text-lg sm:text-xl font-black bg-gradient-to-r from-herb-green-600 via-herb-green-700 to-herb-green-800 bg-clip-text text-transparent truncate">HERBLEDGER</h1>
