...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = () => {
      try {
        const storedUser = localStorage.getItem('herbledger_user');
        const storedToken = localStorage.getItem('herbledger_token');
        ...
        localStorage.removeItem('herbledger_user');
        localStorage.removeItem('herbledger_token');
      } finally {
        setLoading(false);
      }
    };
  ...
  const login = async (email, password, portal = 'farmer') => {
    try {
      ...
      localStorage.setItem('herbledger_user', JSON.stringify(userData));
      localStorage.setItem('herbledger_token', token);
  ...
  const register = async (userData) => {
    try {
      ...
      // Persist to localStorage
      localStorage.setItem('herbledger_user', JSON.stringify(newUser));
      localStorage.setItem('herbledger_token', token);
  ...
  const logout = async () => {
    try {
      ...
      localStorage.removeItem('herbledger_user');
      localStorage.removeItem('herbledger_token');
      localStorage.removeItem('herbledger_refresh_token');
    }
  };
  ...
  const updateProfile = async (profileData) => {
    try {
      ...
      // Update localStorage
      localStorage.setItem('herbledger_user', JSON.stringify(updatedUser));
...
export const useStandaloneAuth = () => {
  ...
  useEffect(() => {
    ...
    const savedUser = localStorage.getItem('herbledgerUser')
    if (savedUser) {
      try {
        setUser(JSON.parse(savedUser))
      } catch (error) {
        console.error('Error parsing saved user:', error)
        localStorage.removeItem('herbledgerUser')
      }
    }
    setLoading(false)
  }, [])
  ...
  const login = async (credentials, portalType) => {
    ...
    try {
      ...
      if (loginField && credentials.password) {
        ...
        const existingUsers = JSON.parse(localStorage.getItem('herbledgerUsers') || '[]')
        let foundUser = null
        ...
        if (foundUser && foundUser.password === credentials.password) {
          ...
          setUser(userData)
          localStorage.setItem('herbledgerUser', JSON.stringify(userData))
          setLoading(false)
  ...
  const signup = async (formData) => {
    ...
    try {
      ...
      localStorage.setItem('herbledgerUsers', JSON.stringify(existingUsers))
  ...
  const updateProfile = async (profileData) => {
    ...
    try {
      ...
      if (userIndex !== -1) {
        ...
        setUser(updatedUser)
        localStorage.setItem('herbledgerUser', JSON.stringify(updatedUser))
      }
