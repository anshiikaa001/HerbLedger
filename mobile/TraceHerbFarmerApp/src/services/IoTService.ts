...
export class IoTService {
  ...
  private mqttConfig = {
    host: 'mqtt.herbledger.com',
    port: 1883,
    protocol: 'mqtt',
    username: 'herbledger-farmer',
  ...
  private deviceConfigurations = {
    'TH-ENV-001': {
      name: 'HERBLEDGER Environmental Sensor',
  ...
  private async initializeMQTT(): Promise<void> {
    try {
      ...
      this.mqttClient.on('connect', () => {
        ...
        this.mqttClient.subscribe('herbledger/sensors/+/data');
        this.mqttClient.subscribe('herbledger/sensors/+/status');
        this.mqttClient.subscribe('herbledger/gps/+/location');
      });
  ...
  private async scanForDevices(): Promise<void> {
    try {
      ...
      // Filter for known HERBLEDGER devices
      const knownDevices = [...bluetoothDevices, ...bleDevices].filter(device => 
        device.name && device.name.includes('HERBLEDGER')
      );

      console.log('🔍 Found HERBLEDGER devices:', knownDevices.length);
