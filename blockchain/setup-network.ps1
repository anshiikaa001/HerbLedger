# HERBLEDGER Blockchain Network Setup - PowerShell Version
...
function Test-Network {
    ...
    # Test orderer
    try {
        $ordererLogs = docker logs orderer.herbledger.com --tail 10 2>&1
    ...
    # Test peers
    $peers = @("peer0.farmers.herbledger.com", "peer0.processors.herbledger.com")
    ...
    # Test CAs
    $cas = @("ca.farmers.herbledger.com", "ca.processors.herbledger.com")
    foreach ($ca in $cas) {
        try {
...
function New-ConnectionProfiles {
    ...
    $farmersProfile = @{
        name = "farmers-herbledger-network"
        version = "1.0.0"
        client = @{
        ...
        organizations = @{
            Farmers = @{
                ...
                certificateAuthorities = @("ca.farmers.herbledger.com")
            }
        }
        orderers = @{
            "orderer.herbledger.com" = @{
                url = "grpcs://localhost:7050"
                grpcOptions = @{
                    "ssl-target-name-override" = "orderer.herbledger.com"
                    "hostnameOverride" = "orderer.herbledger.com"
                }
            }
        }
        peers = @{
            "peer0.farmers.herbledger.com" = @{
                url = "grpcs://localhost:7051"
                grpcOptions = @{
                    "ssl-target-name-override" = "peer0.farmers.herbledger.com"
                    "hostnameOverride" = "peer0.farmers.herbledger.com"
                }
            }
        }
        certificateAuthorities = @{
            "ca.farmers.herbledger.com" = @{
    ...
    $profileDir = "organizations\peerOrganizations\farmers.herbledger.com"
...
function Show-NetworkStatus {
    ...
    docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" --filter "name=herbledger"
    ...
    Write-Host ""
    Write-Success "🎉 HERBLEDGER Blockchain Network is ready!"
...
# Main execution
function Main {
    Write-Status "🌿 Starting HERBLEDGER Blockchain Network Setup"
    Write-Host "===========================================" -ForegroundColor $Blue
