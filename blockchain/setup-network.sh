#!/bin/bash
#
# HERBLEDGER Blockchain Network Setup
...
PeerOrgs:
  - Name: Processors
    Domain: processors.herbledger.com
    EnableNodeOUs: true
    Template:
      Count: 1
    Users:
...
PeerOrgs:
  - Name: Regulators
    Domain: regulators.herbledger.com
    EnableNodeOUs: true
    Template:
      Count: 1
    Users:
...
Organizations:
    - &OrdererOrg
        Name: OrdererOrg
        ID: OrdererMSP
        MSPDir: organizations/ordererOrganizations/herbledger.com/msp
        Policies:
            Readers:
                Type: Signature
        ...
        OrdererEndpoints:
            - orderer.herbledger.com:7050

    - &Farmers
        Name: FarmersMSP
        ID: FarmersMSP
        MSPDir: organizations/peerOrganizations/farmers.herbledger.com/msp
        Policies:
            Readers:
                Type: Signature
        ...
        AnchorPeers:
            - Host: peer0.farmers.herbledger.com
              Port: 7051

    - &Processors
        Name: ProcessorsMSP
        ID: ProcessorsMSP
        MSPDir: organizations/peerOrganizations/processors.herbledger.com/msp
        Policies:
            Readers:
                Type: Signature
        ...
        AnchorPeers:
            - Host: peer0.processors.herbledger.com
              Port: 9051
...
Orderer: &OrdererDefaults
    ...
    EtcdRaft:
        Consenters:
        - Host: orderer.herbledger.com
          Port: 7050
          ClientTLSCert: organizations/ordererOrganizations/herbledger.com/orderers/orderer.herbledger.com/tls/server.crt
          ServerTLSCert: organizations/ordererOrganizations/herbledger.com/orderers/orderer.herbledger.com/tls/server.crt
    BatchTimeout: 2s
    BatchSize:
        MaxMessageCount: 10
...
create_channel() {
    ...
    # Create channel
    ./bin/peer channel create -o localhost:7050 -c herb-channel -f ./channel-artifacts/herb-channel.tx --outputBlock ./channel-artifacts/herb-channel.block --tls --cafile ${PWD}/organizations/ordererOrganizations/herbledger.com/orderers/orderer.herbledger.com/msp/tlscacerts/tlsca.herbledger.com-cert.pem
...
deploy_chaincode() {
    ...
    ./bin/peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.herbledger.com --tls --cafile ${PWD}/organizations/ordererOrganizations/herbledger.com/orderers/orderer.herbledger.com/msp/tlscacerts/tlsca.herbledger.com-cert.pem --channelID herb-channel --name herb-traceability --version 1.0 --package-id $PACKAGE_ID --sequence 1
    ...
    # Commit chaincode
    ./bin/peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.herbledger.com --tls --cafile ${PWD}/organizations/ordererOrganizations/herbledger.com/orderers/orderer.herbledger.com/msp/tlscacerts/tlsca.herbledger.com-cert.pem --channelID herb-channel --name herb-traceability --peerAddresses localhost:7051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/farmers.herbledger.com/peers/peer0.farmers.herbledger.com/tls/ca.crt --peerAddresses localhost:9051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/processors.herbledger.com/peers/peer0.processors.herbledger.com/tls/ca.crt --version 1.0 --sequence 1
...
# Main execution
main() {
    print_status "🌿 Starting HERBLEDGER Blockchain Network Setup"
    ...
    print_success "🎉 HERBLEDGER Blockchain Network setup completed successfully!"
