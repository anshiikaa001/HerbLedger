...
#
# HERBLEDGER Blockchain Network - Crypto Material Generation
...
function New-OrdererCrypto {
    ...
    $ordererDir = "organizations/ordererOrganizations/herbledger.com"
    
    # Create directories
    New-Item -ItemType Directory -Path "$ordererDir/orderers/orderer.herbledger.com/msp" -Force
    New-Item -ItemType Directory -Path "$ordererDir/orderers/orderer.herbledger.com/tls" -Force
    ...
    # Enroll orderer
    docker exec "ca.farmers.herbledger.com" fabric-ca-client enroll -u https://orderer:ordererpw@ca.farmers.herbledger.com:7054 --caname ca-farmers -M /etc/hyperledger/fabric-ca-server/orderer-msp --tls.certfiles /etc/hyperledger/fabric-ca-server/ca-cert.pem
    
    # Copy orderer MSP
    docker cp "ca.farmers.herbledger.com":/etc/hyperledger/fabric-ca-server/orderer-msp/. "$ordererDir/orderers/orderer.herbledger.com/msp/"
    
    # Generate TLS for orderer
    docker exec "ca.farmers.herbledger.com" fabric-ca-client enroll -u https://orderer:ordererpw@ca.farmers.herbledger.com:7054 --caname ca-farmers -M /etc/hyperledger/fabric-ca-server/orderer-tls --enrollment.profile tls --csr.hosts orderer.herbledger.com --csr.hosts localhost --tls.certfiles /etc/hyperledger/fabric-ca-server/ca-cert.pem
    
    # Copy TLS certificates
    docker cp "ca.farmers.herbledger.com":/etc/hyperledger/fabric-ca-server/orderer-tls/. "$ordererDir/orderers/orderer.herbledger.com/tls/"
...
# Main execution
function Main {
    Write-Status "🔐 Starting HERBLEDGER Crypto Material Generation"
    ...
    New-OrgCrypto -OrgName "Farmers" -OrgDomain "farmers.herbledger.com" -CAPort 7054 -MSPId "FarmersMSP"
    New-OrgCrypto -OrgName "Processors" -OrgDomain "processors.herbledger.com" -CAPort 8054 -MSPId "ProcessorsMSP"
    New-OrgCrypto -OrgName "Labs" -OrgDomain "labs.herbledger.com" -CAPort 9054 -MSPId "LabsMSP"
    New-OrgCrypto -OrgName "Regulators" -OrgDomain "regulators.herbledger.com" -CAPort 10054 -MSPId "RegulatorsMSP"
